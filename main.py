...
class MyClient(discord.Client):
    ...
    async def on_message(self, message): 
        ...
        if message.author == 1155392571569356880: 
            if 'test' in message.description:
...
async def configure(interaction: discord.Interaction, role: discord.Role):
    ...
    await channel.send_message(content=f"{pingrole} from now on you will get pinged whenever bumping is possible. Thank you for bumping our server.")
    with open("role.txt", "w", encoding="utf-8") as f:
            f.write(pingrole)
            f.close()
